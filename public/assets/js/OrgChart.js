...
OrgChart.prototype._draw = function (readFromCache, action, actionParams, callback) {
    ...
    this.server.read(readFromCache, this.width(), this.height(), vb, action, actionParams, function (response) {
        ...
        for (var id in response.visibleNodes) {
            ...
            content += that.ui.expandCollapse(that, bgnode, that.config, action, newScale);
        }
...
OrgChart.prototype.isChild = function (from, to) {
    var toNode = this.getBGNode(to);
    var fromNode = this.getBGNode(from);


    if (!toNode.isGroup && !fromNode.isGroup && !toNode.isChildOfGroup && !fromNode.isChildOfGroup) {
        var toNodeParent = this.getBGNode(toNode.pid);

        while (toNodeParent != null) {
            if (toNodeParent.id == from) {
                return true;
            }
            toNodeParent = this.getBGNode(toNodeParent.pid);
        }
    }
...
OrgChart.editUI.prototype._createDetailsView = function (node, detailsViewElement) {
    ...
    Object.assign(photoElement.style, {
        "background-color": "#000000", "min-height": "50px", textAlign: "center", position: "relative"
    });
...
OrgChart.prototype._pagesA100 = function (opt, svg, scale) {
    ...
    var pages = [];
    while (temp_vx < vw - 1) { //it was temp_vx < vw we need temp_vx < vw - 1 becouse we have rounded numbers
        while (temp_vy < vh - 1) { //it was temp_vy < vh we need temp_vy < vh - 1 becouse we have rounded numbers
...
OrgChart.server._createNodes = function (config, action, actionParams, oldNodes) {
    ...
    for (var i = 0; i < assistantPids.length; i++) {
        ...
        if (anodeIds.length % 2) { //odd
            ...
            var oldNode = oldNodes[tnode.id];
            if (oldNode) {
                nodes[tnode.id].state = oldNode.state;
                if (nodes[tnode.id].state == OrgChart.COLLAPSE) {
                    if (collapsedNodeIds.indexOf(tnode.id) == -1) { //Fix: RE: Custom expand/collapse with assistant nodes
                        collapsedNodeIds.push(tnode.id)
                    }
                }
            }
            ...
            OrgChart.server._initStates(tnode, nodes, action, config, actionParams, collapsedNodeIds);
        }
        ...
        for (var j = 0; j < anodeIds.length; j = j + 3) {
            ...
            if (oldNode) {
                ...
                if (nodes[scnode.id].state == OrgChart.COLLAPSE) {
                    if (collapsedNodeIds.indexOf(scnode.id) == -1) { //Fix: RE: Custom expand/collapse with assistant nodes
...
OrgChart.ui = {
    ...
    css: function () {
        var css = '.bg-ripple-container {position: absolute; top: 0; right: 0; bottom: 0; left: 0; }'
            ...
            + 'svg.cursor-grab text:hover {cursor:grab;}'
            + 'svg.cursor-grab {cursor:grab;}'
            + 'svg.cursor-move {cursor:move;}';
    ...
    expandCollapse: function (chart, bgnode, config, action, scale) {
        if (action === OrgChart.action.exporting) {
            return "";
        }
        ...
        if (html.indexOf("{collapsed-children-count}")) {
            var collapsedChildrenCount = OrgChart.collapsedChildrenCount(chart, bgnode);
...
OrgChart.childrenCount = function (chart, node, allChildrenIds) {
    ...
    for (var i = 0; i < childrenIds.length; i++) {
        ...
        if (typeof (id) == "string" && id.indexOf("split") == -1 && id.indexOf("transperant") == -1) {
            if (allChildrenIds.indexOf(id) == -1) {
        ...
        OrgChart.collapsedChildrenCount(chart, chart.nodes[id], allChildrenIds);
    }

    return allChildrenIds.length;
};

OrgChart.collapsedChildrenCount = function (chart, node, allChildrenIds) {
    var childrenIds = node.childrenIds;
    if (allChildrenIds == undefined) {
        allChildrenIds = [];
        childrenIds = node.collapsedChildrenIds;
    }
