...
function CustomSelect({ label, option, onChange, defaultValue }: Props) {
  const [open, setOpen] = useState(false);
  const [options, setOptions] = useState<any[]>([]);
  const [listSelected, setSelected] = useState<any>(null);
  ...
  return (
    <div className="select-custom">
      <div ref={inputContentRef} onClick={() => setOpen((prev) => !prev)} className="select-custom_input">
        <span>{listSelected ? option?.filter((item) => +listSelected === +item.value)?.[0]?.label : `${translate("All")} ${label}`}</span>
        {listSelected ? (
          <div
            className="close"
            onClick={(e) => {
          ...
          <div className="content">
            {options?.length > 0 ? (
              options?.map((item, idx) => (
                <div
                  key={idx}
                  className={`item ${+listSelected === +item.value && "active"}`}
                  onClick={() => {
